...
curl -sL https://deb.nodesource.com/setup20.x | sudo -E bash -
...
sudo rpl -i -w "DB_PASSWORD=linkpanel" "DB_PASSWORD=$DBPASS" /var/www/.env
...
sudo chown -R www-data:linkpanel /var/www
...
sudo chown www-data:linkpanel -R /var/www
...
5 2 * * * cd /var/www/utility/linkpanel-update && sh run.sh >> /dev/null 2>&1
EOF
crontab $TASK
sudo systemctl restart nginx.service
